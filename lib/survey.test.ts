...
void test('multi-select stores a sorted, de-duplicated, filtered list', () => {
  assert.equal(
...
void test('single-select refuses an array, multi-select refuses a bare string', () => {
  assert.equal(cleanAnswer('machine', ['mac']), null);
  assert.equal(cleanAnswer('agentTools', 'mac'), null);
});

void test('goal is trimmed, collapsed and capped', () => {
...
void test('every question has unique choice values', () => {
